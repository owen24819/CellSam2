# Standard library imports
import argparse
import sys
from pathlib import Path

# Third-party imports
import hydra
from cell_tracker import SAM2AutomaticCellTracker
from hydra import compose
from hydra.core.global_hydra import GlobalHydra
from inference_utils import (
    get_device,
    get_result_path,
    get_tif_directories,
    get_video_path,
)
from omegaconf import OmegaConf
import torch

# Local imports
from sam2.build_sam import build_sam2
from sequence_timer import SequenceTimer


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="Cell tracking with SAM2")
    parser.add_argument("--video_path", type=str, default=None,
                        help="Path to video file or image sequence directory")
    parser.add_argument("--res_path", type=str, default=None,
                        help="Path to save tracking results")
    parser.add_argument("--model_name", type=str, default="SAM2-tracking-LoRA-heatmap",
                        help="Name of the model to use")
    parser.add_argument("--box_nms_thresh", type=float, default=0.5,
                        help="Non-maximum suppression threshold for bounding boxes")
    parser.add_argument("--pred_iou_thresh", type=float, default=0.7,
                        help="IoU threshold for predictions")
    parser.add_argument("--div_obj_score_thresh", type=float, default=0.4,
                        help="Division object score threshold")
    parser.add_argument("--obj_score_thresh", type=float, default=0.5,
                        help="Object score threshold")
    parser.add_argument("--segment", action="store_true",
                        help="Whether to perform segmentation (default: False)")
    parser.add_argument("--no_heatmap", action="store_true",
                        help="Disable heatmap (heatmap is used by default)")
    parser.add_argument("--checkpoint_num", type=int, default=None,
                        help="Checkpoint number to use")
    parser.add_argument("--resize_threshold", type=int, default=None,
                        help="Enable tiled inference when images exceed this size")
    parser.add_argument("--crop_overlap", type=int, default=128,
                        help="Overlap in pixels between adjacent crops")
    parser.add_argument("--save_crop_movies", action="store_true",
                        help="Save individual movies for each crop (for debugging)")
    parser.add_argument("--postprocess_divisions", action="store_true",
                        help="Enable post-processing to detect divisions across crops")
    parser.add_argument("--aux_matching", type=str, default="off",
                        choices=["off", "new_cells_only", "segment_then_aux_track"],
                        help="Temporal auxiliary matching mode")
    parser.add_argument("--disable_frame_metrics", action="store_true",
                        help="Disable per-frame metrics CSV output.")
    parser.add_argument("--gt_seg_dir", type=str, default=None,
                        help="Optional CTC GT SEG directory override.")
    parser.add_argument("--save_prediction_videos", action="store_true",
                        help="Save prediction videos.")
    return parser.parse_args()


def setup_hydra(model_name: str):
    """Setup Hydra configuration."""
    config_path = f"../sam2_logs/{model_name}"

    if GlobalHydra().is_initialized():
        GlobalHydra.instance().clear()

    hydra.initialize(version_base=None, config_path=config_path)
    return f"../sam2_logs/{model_name}", "config_resolved"


def _infer_ctc_gt_seg_dir(sequence_dir: Path) -> Path | None:
    gt_seg_dir = sequence_dir.parent / f"{sequence_dir.name}_GT" / "SEG"
    if gt_seg_dir.is_dir():
        return gt_seg_dir
    return None


def _count_tif_frames(dir_path: Path) -> int:
    return sum(1 for p in dir_path.iterdir() if p.suffix.lower() in {".tif", ".tiff"})


def process_directory(
    tracker: SAM2AutomaticCellTracker,
    dir_path: Path,
    base_dir: Path,
    model_name: str,
    input_path: Path,
    res_path: Path,
    total_dirs: int,
    idx: int,
    disable_frame_metrics: bool,
    gt_seg_dir_override: Path | None,
    timer: SequenceTimer,
):
    """Process a single directory with the cell tracker."""
    print(f"Processing directory {idx + 1}/{total_dirs}: {dir_path}")

    result_path = get_result_path(
        base_dir=base_dir,
        model_name=model_name,
        input_path=input_path,
        dir_name=dir_path.stem,
        res_path=res_path,
    )
    result_path.mkdir(parents=True, exist_ok=True)

    num_frames = _count_tif_frames(dir_path)

    torch.cuda.synchronize()
    timer.start()
    tracker.predict(
        dir_path,
        result_path,
        offload_video_to_cpu=True,
        offload_state_to_cpu=True,
        max_frame_num_to_track=None,
        frame_metrics_csv=None if disable_frame_metrics else result_path / "frame_metrics.csv",
        gt_seg_dir=(
            gt_seg_dir_override
            if gt_seg_dir_override is not None
            else _infer_ctc_gt_seg_dir(dir_path)
        ),
    )
    torch.cuda.synchronize()
    timer.stop(sequence_name=dir_path.stem, num_frames=num_frames)

    print(f"Finished processing: {dir_path}")


def main():
    args = parse_args()
    model_name = args.model_name
    config_path, config_name = setup_hydra(model_name)

    device = get_device()
    cfg = compose(config_name=config_name)
    OmegaConf.resolve(cfg)

    if args.resize_threshold is None:
        args.resize_threshold = getattr(cfg.scratch, "resize_threshold", None)

    if args.checkpoint_num is None:
        sam2_checkpoint = f"sam2_logs/{model_name}/checkpoints/checkpoint.pt"
    else:
        sam2_checkpoint = f"sam2_logs/{model_name}/checkpoints/checkpoint_{args.checkpoint_num}.pt"

    sam2_model = build_sam2(config_name, sam2_checkpoint, device=device)

    tracker = SAM2AutomaticCellTracker(
        sam2_model,
        pred_iou_thresh=args.pred_iou_thresh,
        obj_score_thresh=args.obj_score_thresh,
        div_obj_score_thresh=args.div_obj_score_thresh,
        box_nms_thresh=args.box_nms_thresh,
        segment=args.segment,
        use_heatmap=not args.no_heatmap,
        resize_threshold=args.resize_threshold,
        crop_overlap=args.crop_overlap,
        save_crop_movies=args.save_crop_movies,
        postprocess_divisions=args.postprocess_divisions,
        aux_matching=args.aux_matching,
        save_prediction_videos=args.save_prediction_videos,
    )

    video_path = Path(args.video_path) if args.video_path else get_video_path()

    try:
        directories = get_tif_directories(video_path)
        base_dir = Path(__file__).parents[1]

        result_root = Path(args.res_path) if args.res_path else (base_dir / "results")
        timings_csv = result_root / "timings.csv"

        with SequenceTimer(timings_csv) as timer:
            for idx, dir_path in enumerate(directories):
                process_directory(
                    tracker=tracker,
                    dir_path=dir_path,
                    base_dir=base_dir,
                    model_name=model_name,
                    input_path=video_path,
                    res_path=Path(args.res_path) if args.res_path else result_root,
                    total_dirs=len(directories),
                    idx=idx,
                    disable_frame_metrics=args.disable_frame_metrics,
                    gt_seg_dir_override=(
                        Path(args.gt_seg_dir) if args.gt_seg_dir is not None else None
                    ),
                    timer=timer,
                )

        print(f"Saved sequence timings to: {timings_csv}")

    except ValueError as e:
        print(f"Error: {e}")
        sys.exit(1)


if __name__ == "__main__":
    sys.path.append(str(Path(__file__).parent.parent))
    main()