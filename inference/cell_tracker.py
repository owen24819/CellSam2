            ...
            if batch_size > 0:
                ...
                if not self.segment and frame_idx > 0 and self.use_heatmap:
                    ...
                    if input_points.shape[0] > 0:
                        ...
                        # Keep only points inside the content region (drop points in padded region)
                        resized_h, resized_w = inference_state["resized_image_size"]
                        valid = (
                            (input_points[:, 0, 0] >= 0)
                        ...
                        if input_points.shape[0] == 0:
                            pass  # No points left after removing padded region
                        else:
                            ...
                            # Convert to numpy and int; clamp to valid track_mask indices
                            input_points_np = input_points.cpu().numpy().astype(np.int32)
                            h, w = track_mask.shape[:2]
                            y_idx = np.clip(input_points_np[:, 0, 1], 0, h - 1)
                            x_idx = np.clip(input_points_np[:, 0, 0], 0, w - 1)
                            track_cell_ids = track_mask[y_idx, x_idx]
